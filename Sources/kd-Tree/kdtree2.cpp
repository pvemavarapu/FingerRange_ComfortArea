...
namespace kdtree2 {
	...
	KDTree::KDTree(KDTreeArray& data_in,bool rearrange_in,int dim_in)
		...
		data(NULL),
		ind(N){
			std::cout << "BUILDING TREE....DIM = " << dim_in << std::endl;
			sort_results = true;
	...
	KDTreeNode* KDTree::build_tree_for_range(int l, int u, KDTreeNode* parent) {
		...
		if ((u-l) <= bucketsize) {
			...
			// Compute an APPROXIMATE bounding box for this node. if parent == NULL, then this is the root node, and we compute for all dimensions.
			// Otherwise, we copy the bounding box from the parent for all coordinates except for the parent's cut dimension.  
	...
	void KDTreeNode::process_terminal_node(SearchRecord& sr) {
		...
		for (int i=l; i<=u;i++) {
			...
			if (rearrange) {
				...
				// why do we do things like this?  because if we take an early exit (due to distance being too large) which is common, then we need not read in the actual point index, thus saving main
				// memory bandwidth.  If the distance to point is less than the ballsize, though, then we need the index.
				indexofi = sr.ind[i];
			} else {
