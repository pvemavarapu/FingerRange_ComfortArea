...
//!
//! \brief Object display class (WIP).
//|___________________________________________________________________
...
bool ObjectDisplay::createSceneObject(gmtl::Point3f size, gmtl::Vec4f color){
	...
	//
	osg::ref_ptr<osg::DrawPixels> drawPixels = new osg::DrawPixels;

	osg::ref_ptr<osg::Geode> objectMarker = new osg::Geode;
	osg::ref_ptr<osg::ShapeDrawable> objectShape = new osg::ShapeDrawable(new osg::Box(osg::Vec3(0.0f, 0.0f, 0.0f), size[0], size[1], size[2]));
		
	//osg::ShapeDrawable(new osg::Box(osg::Vec3(0.0f, 0.0f, 0.0f), PIXEL_SIZE_ON_SCREEN, size[1], size[2]));

	osg::StateSet* state = objectMarker->getOrCreateStateSet();
	...
	objectMarker->addDrawable(objectShape.get());
	...
	_object_tf_nd->addChild(objectMarker.get());
...
void ObjectDisplay::makeTransparent(){
	_fade_Objects = true;
	osg::ref_ptr<osg::Node> _object_Marker = _object_tf_nd->getChild(0);
	osg::StateSet* state = _object_Marker->getOrCreateStateSet();
...
void ObjectDisplay::setScale(const float x, const float y, const float z, const float tx, const float ty, const float tz){
	...
	matS.makeScale(osg::Vec3f(1, 1, 1));
	matR.identity();
...
//| Function: select
void ObjectDisplay::select(const bool select_status){
	//if (select_status) {    
	//	objectNameTextNode->setCharacterSize(12000.0f);
	//	pane->setColor(0.0f, 1.0f, 0.0f, 0.5f);
	//} else {    
	//	objectNameTextNode->setCharacterSize(3000.0f);
	//}
}